...
def analyze_market_context(symbol="XAUUSD", timeframe="1h", model_name="llama3", strategy_name="ICT 2022 Model"):
    """
    Dual-AI Pipeline:
    ...
    # Inject the raw candles for the new modular strategy framework
    final_output["raw_candles"] = mtf_data_raw.get(timeframe, [])
    
    setup_scenario = market_data.build_trade_scenarios(final_output, confluence['bias'])
    final_output["deterministic_scenario"] = setup_scenario
    ...
    if OLLAMA_AVAILABLE:
        try:
            prompt = f"""
You are the explanation layer of a highly disciplined deterministic trading engine.
...
"""
            ...
            disclaimer = f"[AI] Explanation generated by Local Ollama ({model_name})."
    ...
    final_output["disclaimer"] = disclaimer
