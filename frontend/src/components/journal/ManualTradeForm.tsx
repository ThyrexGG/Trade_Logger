...
import { createManualTrade } from '../../api/operations'
...
interface Props {
  ...
  /** Called with the newly-created row so the caller can drop it straight
 ...
 */
export function ManualTradeForm({ knownAccounts, onCreated }: Props) {
    ...
    >
      <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
        <label className="flex flex-col gap-1 text-[11px] text-muted">
          ...
          />
          <datalist id="manual-trade-accounts">
            {knownAccounts.map((a) => (
        ...
        <label className="flex flex-col gap-1 text-[11px] text-muted">
          ...
          <input
            type="text" list="manual-trade-setups" value={setupTag} onChange={(e) => setSetupTag(e.target.value)}
            className="rounded border border-border bg-background px-2 py-1 text-xs text-primary focus:border-accent focus:outline-none"
          />
          <datalist id="manual-trade-setups">
            {SETUP_PRESETS.map((p) => (
