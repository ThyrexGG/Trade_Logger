...
FAKE_CREDS = {"api_key": "fake-api-key", "email": "fixture@example.test", "password": "fake-fixture-password"}
...
def test_roundtrip_and_ciphertext_opaque(enc):
    with tenant.use("alice"):
        meta = bc.create_connection(secret=FAKE_CREDS, account_id="A1", is_demo=True, label="demo")
        got = bc.get_secret(meta["id"])
    assert got["api_key"] == FAKE_CREDS["api_key"]
    assert got["password"] == FAKE_CREDS["password"]
...
def test_connections_are_tenant_scoped(enc):
    with tenant.use("alice"):
        a = bc.create_connection(secret=FAKE_CREDS, account_id="A1")
    with tenant.use("bob"):
...
def test_partial_secret_update_merges(enc):
    with tenant.use("alice"):
        m = bc.create_connection(secret=FAKE_CREDS, account_id="A1")
        bc.update_connection(m["id"], secret={"password": "fake-new-pass"})
        got = bc.get_secret(m["id"])
    assert got["password"] == "fake-new-pass"
    assert got["api_key"] == FAKE_CREDS["api_key"]     # untouched
    assert got["email"] == FAKE_CREDS["email"]
...
def test_active_connections_all_users(enc):
    with tenant.use("alice"):
        bc.create_connection(secret=FAKE_CREDS, account_id="A1")
    with tenant.use("bob"):
        b = bc.create_connection(secret={"api_key": "fake-bk", "email": "b@example.test", "password": "fake-bpw"},
                                 account_id="B1")
    ...
    assert alice["secret"]["api_key"] == "fake-api-key"
...
def supa(monkeypatch, enc):
    jwt_secret = "fake-fixture-jwt-secret"
...
def test_http_connections_isolated(supa):
    ...
    r = c.post("/api/connections", headers=ah, json={
        ...
        "secret": {"api_key": "fake-ak", "email": "a@example.test", "password": "fake-pw"}})
    assert r.status_code == 201, r.text
    cid = r.json()["connection"]["id"]
    assert "secret" not in json.dumps(r.json())
    assert "fake-ak" not in json.dumps(r.json())
