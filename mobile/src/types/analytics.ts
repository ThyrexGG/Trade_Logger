/**
 * Analytics contracts (`/api/analytics/performance`, `/api/analytics/day`).
 * Hand-copied from frontend/src/types/analytics.ts; read-only — every metric is
 * computed by the backend, the phone only formats.
 */
...
export interface AnalyticsAvailable {
  ...
  /** The user's own saved value for this account, if they've set one — always wins over the suggestion above. */
  saved_initial_balance: number | null
}
