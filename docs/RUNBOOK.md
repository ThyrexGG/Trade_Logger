...
min, ~07:00-23:00 Cambodia time (00:00-15:50 UTC), so Render doesn't cold-start
(~50 s) on the first visit. It sleeps overnight on purpose (Render free caps
total run time at ~750 h/month) and never marks a run failed on a transient
miss. Wake it on demand from the repo's **Actions** tab -> keep-warm -> **Run
workflow**. Widen the `cron` hour range if friends are in other timezones. The
real fix for cold starts is Render **Starter ($7/mo)** — no sleep, more CPU.
