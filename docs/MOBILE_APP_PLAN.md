...
- **Phase 2 (owner-approved 2026-09-19)** — bring more of the website to the phone. In: Analytics (+ calendar), Price Alerts, Manual trade entry, Risk Gateway, AI Assistant, Killzone Scanner. Owner first left out Challenge Tracker and Chart Analyzer, then asked for them on 2026-09-20 (M20, M21); not planned: Command Center, Market, Research pages, System/Connections/Partners. Tabs are now Positions · Journal · Analytics · More (Account moved under More).
  ...
  - [x] M23 Weekly summary (2026-09-21): `api/weekly_summary.py` + `/api/weekly-summary` — Monday-Sunday (UTC) net, win rate, best/worst trade, best/costliest setup, per-day; one `summary` notification per user per week from Sunday 12:00 UTC (event key per user+week; a missed one is sent late inside a 3-day grace; a week with no closed trades sends nothing; on by default, per-user switch). Checked from every sync cycle and the background loop (now also watches anyone with a registered phone). Phone: More → Weekly summary. Needs build 4.
  - [x] M24 Setups + faster tagging (2026-09-21): More → Setups — tagged X of Y, your record per setup (BEST / too-few-trades hints), and "Tag the rest": untagged closed trades with one-tap chips. Tag chips everywhere fall back to the ten starter setups (`journal/presets.ts`, same list as the website). `/journal/tag-stats` now also returns the untagged count. Needs build 4.
  - [x] M25 Partial closes grouped (2026-09-22): `api/trade_groups.py`. Capital.com stores each partial close as its own `closed_trades` row (`<deal>`, `<deal>_1`, `<deal>_2`…), so one scaled-out trade looked like several trades that each needed notes and a tag. The journal now returns ONE entry per position: the row of the last exit is the main trade (it keeps notes/tag/screenshots), its net/commission/swap are the position totals, and `legs` lists every exit oldest-first — earlier ones "partial", the last "final" (all "partial" + `position_open` while the position is still open at the broker). MT5 already merged a position into one row on its last close; its exit legs now come from `raw_deals` (entry fees spread by volume so legs add up to the net). `/journal` counts, `/journal/tag-stats` and the PATCH reply use positions; a link to an old partial id opens its main trade. A Capital partial's own size and price are not shown (the broker repeats the last exit's on every partial row) — only its time and P&L. Analytics, the Command Center and the weekly summary now fold the same way (`trade_groups.folded_dataframe`), so trade counts and win rate agree everywhere (checked against the owner's real account: 115 trades / 33.0% win rate → 105 trades / 28.6%, net P&L unchanged). The calendar (which reads Analytics' day/performance data) is fixed by the same change. The AI Assistant's tool queries (`api/ai_tools.py`) fold the same way too. Website live on push; phone Exits card + pill need build 4.
  - [x] M26 Multi-account labeling (2026-09-23): the owner linked a real MT5 account alongside their existing Capital.com one — the first time any account had two real linked accounts — and every screen that showed a raw `account_id` (Journal rows, the account chip row, Trade/Position detail, the day-trades list, Loss limits, the manual-trade account picker, Positions cards, the Challenge screen's account chips) showed the opaque broker id verbatim. New shared `accountLabel.ts` + `<AccountTag>` turn it into "MT5 14271408" / "Capital.com ••6500"; unrecognized ids (a manual/demo label) are shown as typed rather than guessed. Positions also gained a per-card account tag (had none before) — mirrors the website's `lib/accountLabel.ts` / `AccountBadge`, same regex. The Analytics screen's account chip row also now remembers your last pick (AsyncStorage) and defaults to Capital.com the very first time, mirroring the website's A27 fix. Needs build 4.
...
- **Build 3 (2026-09-20)** — EAS preview APK with M10–M22 and every emulator/checklist fix: https://expo.dev/artifacts/eas/3pq4yuVIoVDtxFed-MTj5NBFa6122Skl1OB2RTMtllM.apk (build page: https://expo.dev/accounts/thyrexgg/projects/tradelogger/builds/80dad154-4bce-4a3d-829d-0597e5a93b20). Not yet installed on a real phone.
- **Website parity + smaller uploads (2026-09-20)** — tradelogger.site got the two things that were phone-only: a **Loss limits** page (`/workspace/loss-limits`) and **Edit details / Delete** on hand-logged (`MANUAL_`) trades in the Journal (feed + table). `.easignore` at the repo root makes the EAS upload only `mobile/` (was ~633 MB — `mobile/node_modules` + git history because EAS ignores the nested `mobile/.gitignore`; checked with `eas build:inspect --stage archive`: 1.2 MB, 124 files). Takes effect on the next build.
