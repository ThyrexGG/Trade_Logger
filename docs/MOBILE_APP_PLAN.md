...
- **Phase 2 (owner-approved 2026-09-19)** — bring more of the website to the phone. In: Analytics (+ calendar), Price Alerts, Manual trade entry, Risk Gateway, AI Assistant, Killzone Scanner. Owner first left out Challenge Tracker and Chart Analyzer, then asked for them on 2026-09-20 (M20, M21); not planned: Command Center, Market, Research pages, System/Connections/Partners. Tabs are now Positions · Journal · Analytics · More (Account moved under More).
  ...
  - [x] M26 Multi-account labeling (2026-09-23): the owner linked a real MT5 account alongside their existing Capital.com one — the first time any account had two real linked accounts — and every screen that showed a raw `account_id` (Journal rows, the account chip row, Trade/Position detail, the day-trades list, Loss limits, the manual-trade account picker, Positions cards) showed the opaque broker id verbatim. New shared `accountLabel.ts` + `<AccountTag>` turn it into "MT5 14271408" / "Capital.com ••6500"; unrecognized ids (a manual/demo label) are shown as typed rather than guessed. Positions also gained a per-card account tag (had none before) — mirrors the website's `lib/accountLabel.ts` / `AccountBadge`, same regex. Needs build 4.
