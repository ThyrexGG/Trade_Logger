...
| Area | State |
| :-- | :-- |
| Real Intraday Provider (MT5) & Native Gold Revalidation (Phase 74) — `mt5_provider.py` + `dataset_manifest.py` + `native_gold_revalidation.py` (rewritten) + `strategies/smc_utils.py` (§33 optimisation) | **research-only**, read-only. `MT5Provider` pulls the account's own terminal (env credentials): real broker XAUUSD **spot** — 1m ≈ 3.4 mo (100 k-bar cap), 5m ≈ 17 mo, 15m ≈ 4.2 y, 1h ≈ 10 y; FX universe M15 ingested. Native Gold revalidation **ran on independent broker data → NO NATIVE EDGE** (−0.092R / N=1 067; frozen holdout untouched, not a delta). SMC detectors ≈105× faster, 0 mismatches. `GET /api/research/dataset-manifest`; `python -m {market_data_ingest --provider mt5, dataset_manifest, native_gold_revalidation}`. Final research answer: **NO_VALIDATED_EDGE**. See `docs/PHASE_74_INTRADAY_PROVIDER.md`. |
| Intraday Data Foundation & Native Gold (Phase 73) — `historical_provider.py` + `data_coverage.py` + `native_gold_revalidation.py` | **research-only**, read-only. Formal intraday provider protocol + capability layer; coverage report (SUFFICIENT/PARTIAL/INSUFFICIENT_DATA). Superseded by Phase 74's real MT5 provider. `GET /api/research/{data-coverage,historical/providers,gold-revalidation/native}`. See `docs/PHASE_73_INTRADAY_DATA.md`. |
